...
_, bw = cv2.threshold(image_gray, int(${detail} * 255), 255, cv2.THRESH_BINARY_INV)
...
{#if pyodideReady}
	<main
		class="min-h-screen bg-gray-800 text-white dark:bg-gray-800 dark:text-white px-4 py-8"
	>
		<div class="relative max-w-xl mx-auto">
			...
			{#if showSettings}
				<div class="mb-4 p-4 rounded-md border border-gray-600 bg-gray-700">
					<label class="block text-sm mb-1"
						>Stencil Detail: {Math.round(detail * 100)}%</label
					>
					<input
						type="range"
						min="0"
			...
			<div class="mt-8 space-y-8">
				{#if outputs.cleaned}
					<div>
						<h2 class="text-xl font-semibold mb-2 text-pink-400">
							Cleaned
						</h2>
						<img
							src={outputs.cleaned}
						...
						/>
						<a
							href={outputs.cleaned}
							download="cleaned.png"
							class="mt-2 inline-block text-pink-400 hover:underline"
							>Download / Open In…</a
						>
					</div>
				{/if}
...
{:else}
	<div
		class="flex items-center justify-center min-h-screen bg-gray-800 text-white"
	>
		<div class="text-center">
			<svg
				class="animate-spin h-12 w-12 text-pink-500 mx-auto mb-4"
